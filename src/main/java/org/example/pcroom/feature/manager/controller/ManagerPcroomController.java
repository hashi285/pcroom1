...
public class ManagerPcroomController {
    ...
    public ResponseEntity<List<ManagerPcroomDto.FindHourlyUtilization>> getHourlyUtilization(Authentication authentication, @RequestParam(defaultValue = "24") int hours // 기본 최근 24시간
    ) {
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
    ...
    public ResponseEntity<List<ManagerPcroomDto.FindHourlyUtilization>> getHourlyUtilization(Authentication authentication, @RequestParam(defaultValue = "24") int hours, @RequestParam(required = false) List<Long> pcroomId // 선택적으로 특정 피시방만
    ) {
        CustomUserDetails userDetails = (CustomUserDetails) authentication.getPrincipal();
        ...
        if (pcroomId != null && !pcroomId.isEmpty()) {
            ...
            pcroomIdsToQuery = registeredPcroomIds.stream().filter(pcroomId::contains).toList();
        } else {
